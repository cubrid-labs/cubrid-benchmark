...
const TABLE = "bench_driver_cubrid_ts_extended"
...
async function runConnectDisconnect(): Promise<void> {
  for (let i = 0; i < 100; i += 1) {
    const workerConn = await getCubridConnection()
    await cubridQuery(workerConn, "SELECT 1")
    await cubridClose(workerConn)
  }
}
...
async function runPreparedStatement(): Promise<void> {
  ...
  for (let i = 1; i <= 1000; i += 1) {
    await cubridQuery(conn, `SELECT id, name, amount FROM ${TABLE} WHERE amount = ?`, [i])
  }
}
...
async function runBatchInsert(): Promise<void> {
  ...
  await cubridQuery(conn, `INSERT INTO ${TABLE} (name, amount) VALUES ${placeholders}`, params)
}
    ...
    ranges.map(async ([start, end]) => {
      ...
      try {
        for (let value = start; value <= end; value += 1) {
          await cubridQuery(workerConn, `SELECT id, name, amount FROM ${TABLE} WHERE amount = ?`, [value])
        }
      } finally {
        await cubridClose(workerConn)
      }
...
test("cubrid tier1 extended throughput", async () => {
  ...
  await writeBenchmarkJson("typescript_tier1_extended_cubrid.json", {
    database: "cubrid",
    tier: 1,
    rounds: 3,
    warmupRounds: 1,
