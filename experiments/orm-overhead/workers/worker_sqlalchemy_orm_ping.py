#!/usr/bin/env python3
"""benchflow external worker for SQLAlchemy ORM ping hot-path benchmarking."""
...
OPERATION_NAMES = ("session_select_by_pk",)
...
def run_worker(input_obj: WorkerInput) -> dict[str, Any]:
    ...
    with Session(engine) as session:
        session.add_all(
            [
                BenchItem(id=row_id, name="seed_{0}".format(row_id), amount=row_id)
    ...
    def op_session_select_by_pk() -> None:
        nonlocal query_calls
        with Session(engine) as session:
            query_calls += 1
            session.get(BenchItem, 1)

    step_durations["session_select_by_pk"] = _run_step(
        collectors["session_select_by_pk"],
        op_session_select_by_pk,
        input_obj.warmup_s,
        input_obj.duration_s,
    )
...
def main() -> None:
    if len(sys.argv) < 2:
        _fatal("usage: worker_sqlalchemy_orm_ping.py <config.json>")
    try:
